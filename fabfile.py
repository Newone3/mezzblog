...
def install():
    ...
    # sudo("yum update -y -q")
    # yum("epel-release")
    # yum("nginx libjpeg-turbo libjpeg-turbo-devel python-pip python-devel python-setuptools git "
    #     "postgresql-server postgresql-devel postgresql-contrib gcc memcached rsync")

    # Enable and start rsyncd and memached daemons
    sudo("systemctl enable rsyncd; systemctl start rsyncd")
    sudo("systemctl enable memcached; systemctl start memcached")

    # Open rsyncd and http ports
    sudo("firewall-cmd --add-service=rsyncd --permanent; firewall-cmd --reload")
    sudo("firewall-cmd --add-service=http --permanent; firewall-cmd --reload")

    # Open https port if SSL enabled
    if not env.ssl_disabled:
        sudo("firewall-cmd --add-service=https --permanent; firewall-cmd --reload")

    # Change home directory to more permissive than Red Hat default 700, due to umask 077
    sudo("chmod 0711 /home/%s" % env.user)
    ...
    run("echo 'export WORKON_HOME=%s' >> /home/%s/.bashrc" % (env.venv_home,
                                                              env.user))
    run("echo 'source /usr/bin/virtualenvwrapper.sh' >> "
